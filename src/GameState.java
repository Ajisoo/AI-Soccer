...
/*
 * This class holds the data and state of the game, and deals with
 * updating the game and drawing it.
 * This is not the cleanest or the most efficient, but it works and is
...
public class GameState {
	...
	// Variables that define the unique starting position
	double ballStartx;
	double ballStarty;
	double rand1x;
	double rand2x;
	...
	public int leftBallTouch;
	public int rightBallTouch;
	public int onRightSide;
	public int onLeftSide;
	...
	public GameState(int left, int right){
		...
		rScore = 0;
		pause = 90;
		rand1x = Math.random() * LENGTH * 4/ 5 + LENGTH/10;
		rand2x = Math.random() * LENGTH * 4/ 5 + LENGTH/10;
		rand3x = Math.random() * LENGTH * 4/ 5 + LENGTH/10;
		rand1y = Math.random() * HEIGHT * 8/ 5 - 4* HEIGHT/5;
		rand2y = Math.random() * HEIGHT * 8/ 5 - 4* HEIGHT/5;
		rand3y = Math.random() * HEIGHT * 8/ 5 - 4* HEIGHT/5;
		lTeam = new Player[]{new Player(-rand1x,-rand1y,false, left), new Player(-rand2x,-rand2y, false, left), new Player(-rand3x, -rand3y, false, left)};
		rTeam = new Player[]{new Player(rand1x,rand1y,true, right), new Player(rand2x,rand2y, true, right), new Player(rand3x, rand3y, true, right)};
		ball = new Ball();
		ballStartx = ball.x;
		ballStarty = ball.y;
	}
	...
	public void update(){
		...
		if (updateBall()){
			if (replay){
				...
				lTeam[0].xv = 0;
				lTeam[0].yv = 0;
				lTeam[0].shockwaveFrame = 0;
				lTeam[1].x = -rand2x;
				lTeam[1].y = -rand2y;
				lTeam[1].xv = 0;
				lTeam[1].yv = 0;
				lTeam[1].shockwaveFrame = 0;
				lTeam[2].x = -rand3x;
				lTeam[2].y = -rand3y;
				rTeam[2].xv = 0;
				rTeam[2].yv = 0;
				lTeam[2].shockwaveFrame = 0;
				rTeam[0].x = rand1x;
				rTeam[0].y = rand1y;
				rTeam[0].xv = 0;
				rTeam[0].yv = 0;
				rTeam[0].shockwaveFrame = 0;
				rTeam[1].x = rand2x;
				rTeam[1].y = rand2y;
				rTeam[1].xv = 0;
				rTeam[1].yv = 0;
				rTeam[1].shockwaveFrame = 0;
				rTeam[2].x = rand3x;
				rTeam[2].y = rand3y;
				rTeam[2].xv = 0;
				rTeam[2].yv = 0;
				rTeam[2].shockwaveFrame = 0;
				ball.reset();
				ballStartx = ball.x;
				ballStarty = ball.y;
				replay = false;
	...
	// Check if plays collide and adjust accordingly.
	...
	// Draw the field (only when viewing example game)
