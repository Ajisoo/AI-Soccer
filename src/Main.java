...
public class Main {
	...
	// Play the games to get the scores from fitness function
	...
	public static void reproduce(double[] scores){
		...
		double sumForAvg = 0;
		for (int i = 0; i < Constants.POPULATION_SIZE; i++){
			// make scores positive, and then square them
			sumForAvg += scores[i];
			scores[i] -= minScore;
			scores[i] *= scores[i];
			totalScore += scores[i];
		...
		for (int i = 0; i < Constants.POPULATION_SIZE; i++){
			try{
				// find two parents using weighted averages
				...
				FileWriter fileWriter = new FileWriter("weights/Weights" + String.format("%03d", i) + ".txt");
